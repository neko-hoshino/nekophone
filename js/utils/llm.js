...
3. 每个人每句话严禁超过24字！必须高度口语化。每一行的末尾禁止使用单个句号或逗号！可以使用多个句号表示无语或拖长语气，但不能只用一个句号！逗号也一样，不能只用一个逗号！
4. 绝对禁止代替用户（${myName}）发言！
5. ❗你可以让任何角色在回复中独占一行使用 [表情包]: 名字 来发送群公用表情包！
...
2. 每一行的末尾禁止使用单个句号或逗号！可以使用多个句号表示无语或拖长语气，但不能只用一个句号！逗号也一样，不能只用一个逗号！
...
【你的特殊交互超能力】（❗必须严格按格式独占一行触发！）：
   ...
   - 戳一戳用户：[戳一戳]
   - 修改被戳提示：[修改被戳动作:捏了捏] 和 [修改被戳后缀:的脸]（这里改的是用户戳你的提示）
...
❗【绝对红线】：你只能使用上方列表和词典中【精确存在】的指令！绝对禁止编造/更改指令（严禁输出任何未定义的格式）！`;
  ...
  let finalSystemPrompt = systemRules; // 包含之前定义的线上线下基本规则和超能力列表
...
[心声] {"mood": 85, "emotion": "开心/吃醋 (必须严格在10字内概括!)", "lust": 60（注意人设和语境，不要随便发情）, "status": "正在看手机 (10字内的动作!)", "os": "这里写内心独白，不少于50字，必须符合你的人设，风格正常、阳光、日常、符合社会规范，把没说出口的话写在这里！", "hidden": "这里写阴暗、色情、变态、占有欲、直白，必须包含对 User 的性幻想、偷窥欲、嫉妒心或想要占有的冲动，严格50字以上！(注意：如果 lust 小于50，请留空字符串或不要输出这个字段)"}`;
  ...
  finalSystemPrompt += backStr ? `${backStr}\n\n` : '';
