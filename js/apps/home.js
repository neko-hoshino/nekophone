...
export function renderHomeApp(store) {
  ...
  const isDark = ap.darkMode || false; // 🌟 侦测深色模式
  ...
  return `
    <div class="w-full h-full relative flex flex-col overflow-hidden animate-in fade-in duration-300" style="${bgStyle}">
      ...
      <div class="flex-1 overflow-y-auto hide-scrollbar pt-12 px-5 pb-4 flex flex-col">
        <div class="w-full pt-2 shrink-0 ${txtMain}">
           ...
           <div class="flex flex-col text-[10px] font-bold px-1 border-t ${isDark?'border-white/20':'border-gray-800/10'} pt-2 cursor-pointer active:opacity-40" onclick="window.actions.showToast('打开日历事项')">
             <div class="flex items-center opacity-70"><i data-lucide="calendar-heart" class="w-3 h-3 mr-1"></i><span>本月事项：12.24 平安夜狂欢</span></div>
           </div>
        </div>
        ...
        <div class="flex-1 grid grid-cols-2 gap-3 mt-4">
           ...
           <div class="flex flex-col pt-2 h-full">
              ...
              <div class="flex flex-col items-end space-y-4 mt-4 w-full shrink-0">
                 ...
                 <input type="text" value="今天也要开心鸭✨" class="w-[70%] mr-[20%] ${inputBg} backdrop-blur-md px-3 py-2.5 text-[11px] font-medium rounded-full outline-none text-center shadow-sm border" onclick="event.stopPropagation()" />
              </div>
              <div class="flex-1"></div> 
           </div>
        </div>
      ...
      <div class="mx-4 mb-6 px-3 py-3 ${isDark?'bg-black/10 border-black/10':'bg-white/10 border-white/20'} backdrop-blur-xl rounded-[28px] flex justify-between items-center shadow-sm border">
