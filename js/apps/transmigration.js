...
【输出规则】严格输出 JSON：
{
  "narration": "叙述文本（200字以上，第二人称，沉浸叙事，禁止'你是X'句式）",
  "choices": ["行动选项1","行动选项2","行动选项3","行动选项4（至少3个）"],
...
function renderActiveDungeon() {
    ...
    : dungeon.completed
      ...
      : dungeon.choices.map((c, i) => `
        <button onclick="window.transActions.dungeonChoose(${i})"
          ...
          style="background:#161b22;border:1px solid rgba(255,255,255,0.07);color:#c9d1d9;">
          <span style="color:#58d4f5;font-weight:700;">▸</span> ${c}
        </button>`).join('');

  // 完成覆盖层
  const completionOverlay = dungeon.completed ? `
  <div class="absolute inset-0 z-20 flex items-center justify-center p-5"
    ...
    <div class="w-full rounded-2xl p-6 text-center" style="background:#161b22;border:1px solid rgba(63,185,80,0.4);">
      <div class="text-4xl mb-2">🎉</div>
      <div class="text-[9px] font-mono mb-1" style="color:#3fb950;letter-spacing:3px;">MISSION COMPLETE</div>
      ...
      <div class="flex justify-center gap-8 mb-5">
        <div><div class="text-[22px] font-black font-mono" style="color:#58d4f5;">+${dungeon.mission.reward?.points || 200}</div><div class="text-[10px]" style="color:#8b949e;">积分</div></div>
        <div><div class="text-[22px] font-black font-mono" style="color:#bc8cff;">+${dungeon.mission.reward?.exp || 50}</div><div class="text-[10px]" style="color:#8b949e;">经验</div></div>
      </div>
...
window.transActions = {
  ...
  async startPresetDungeon(missionId) {
    ...
    const dungeon = {
      ...
      choices:  script.initial_choices || ['观察四周', '谨慎前行', '寻找信息'],
  ...
  async dungeonChoose(idx) {
    ...
    const action = dungeon.choices[idx];
    if (!action) return;
    await _processDungeonTurn(action);
  },
  ...
  completeDungeon() {
    ...
    initData();
    const player = store.transData.player;
    const r = dungeon.mission.reward || { points: 200, exp: 50 };
...
async function _processDungeonTurn(actionText) {
  const dungeon = window.transState.activeDungeon;
  if (!dungeon || window.transState.generatingTurn || dungeon.completed) return;
  ...
  try {
    ...
    // 判定完成（AI判定 OR 进度满 OR 生存归零）
    if (result.is_completed || dungeon.status.progress >= 100) {
      dungeon.completed     = true;
      dungeon.completionMsg = result.completion_message || '你成功完成了副本任务，系统为你结算奖励。';
    } else if (dungeon.status.survival <= 0) {
      dungeon.completed     = true;
      dungeon.completionMsg = '生存值归零，任务失败——但这也是一段珍贵的经历。';
